...
import type { MarketDepthQuote } from "@/lib/angelone";
...
export default function MarketDepthModal({
  ...
  useEffect(() => {
    load(false);
    const id = setInterval(() => load(true), 4_000);
    ...
    <div className="md-backdrop" onClick={onClose} role="presentation">
      <div className="md-modal" onClick={e => e.stopPropagation()} role="dialog" aria-label="Market depth">
        <div className="md-head">
          ...
          <button type="button" className="md-close" onClick={onClose} aria-label="Close">×</button>
        </div>
        ...
        <div className="md-symbol-row">
          ...
          <div className="md-symbol-actions">
            {onOpenChart && (
              <button type="button" className="md-icon-btn" title="Chart" onClick={onOpenChart}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 3v18h18"/><path d="M7 14l4-4 3 3 5-6"/></svg>
              </button>
            )}
          </div>
        </div>
