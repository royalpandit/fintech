...
function parseBuySellBody(body: Record<string, unknown>) {
  return {
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  try {
    const portfolio = await competitionTradingRepository.buyStock(
      Number(id),
      auth.userId,
      parseBuySellBody(body),
    );
    return NextResponse.json({ ok: true, data: serializePortfolio(portfolio!) });
  } catch (e) {
    return NextResponse.json(
      { ok: false, error: e instanceof Error ? e.message : "Buy failed" },
      { status: 400 },
    );
  }
}
