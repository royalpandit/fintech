import { NextResponse, type NextRequest } from "next/server";
import { requireRole } from "@/lib/auth";
import { serializeBenchmark } from "@/lib/finuer-basket";
import { finuerBasketRepository } from "@/lib/finuer-basket-repository";

export const dynamic = "force-dynamic";

type Ctx = { params: Promise<{ id: string }> };

/** PUT /api/v1/admin/benchmarks/:id */
export async function PUT(req: NextRequest, ctx: Ctx) {
  const auth = await requireRole(req, ["super_admin"]);
  if (!auth) return NextResponse.json({ ok: false, error: "Forbidden" }, { status: 403 });

  const id = Number((await ctx.params).id);
  if (!Number.isFinite(id)) return NextResponse.json({ ok: false, error: "Invalid id" }, { status: 400 });

  const body = await req.json();
  const data: { marketId?: number; name?: string } = {};
  if (body.marketId != null || body.market_id != null) {
    data.marketId = Number(body.marketId ?? body.market_id);
  }
  if (body.name !== undefined) data.name = String(body.name);

  try {
    const benchmark = await finuerBasketRepository.updateBenchmark(id, data);
    return NextResponse.json({ ok: true, data: serializeBenchmark(benchmark) });
  } catch {
    return NextResponse.json({ ok: false, error: "Benchmark not found" }, { status: 404 });
  }
}

/** DELETE /api/v1/admin/benchmarks/:id */
export async function DELETE(req: NextRequest, ctx: Ctx) {
  const auth = await requireRole(req, ["super_admin"]);
  if (!auth) return NextResponse.json({ ok: false, error: "Forbidden" }, { status: 403 });

  const id = Number((await ctx.params).id);
  if (!Number.isFinite(id)) return NextResponse.json({ ok: false, error: "Invalid id" }, { status: 400 });

  try {
    await finuerBasketRepository.deleteBenchmark(id);
    return NextResponse.json({ ok: true });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Failed to delete benchmark";
    return NextResponse.json({ ok: false, error: msg }, { status: 400 });
  }
}
