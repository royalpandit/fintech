...
import { getBulkDeals } from "@/lib/nse-market-snapshots";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const { rows, fetchedAt } = await getBulkDeals();
    return NextResponse.json({ ok: true, rows, fetchedAt });
  } catch (e) {
    return NextResponse.json(
      { ok: false, error: e instanceof Error ? e.message : "Bulk deals unavailable", rows: [] },
      { status: 502 },
    );
  }
}
